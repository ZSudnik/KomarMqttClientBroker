...
/**
 * Hue side bar Component that invokes onValueChange when the value is mutated.
 *
 * @param modifier modifiers to set to the hue bar.
 * @param currentColor the initial color to set on the hue bar.
 * @param onValueChange the callback that is invoked when hue value changes. Hue is between 0 - 360.
 */
@Composable
fun HueBar(
        ...
        modifier = modifier
            ...
            .pointerInput(Unit) {
                awaitEachGesture {
                    ...
                    onValueChange(getHueFromPoint(down.position.y, size.height.toFloat()))
                    drag(down.id) { change ->
                        if (change.positionChange() != Offset.Zero) change.consume()
                        onValueChange(getHueFromPoint(change.position.y, size.height.toFloat()))
                    }
                }
            }
    ) {
